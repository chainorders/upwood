    ...
    let (wrapped_amount, wrapped_token) = {
        ...
        match fund.state {
            FundState::Open => {
                ...
                let security_amount = rate_convert_currency_to_security(&fund.rate, &curr_amount)?;
...
fn claim_investment(
    ...
    for investment in params.investments {
        let (token, currency_token, currency_amount, security_amount, fund_state) = {
            ...
            let security_amount = rate_convert_currency_to_security(&fund.rate, &currency_amount)?;
            (
                fund.token.clone(),
                currency_token,
                currency_amount,
