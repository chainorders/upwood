...
interface TransferHolderBalancePopupProps {
	open: boolean;
	onClose: () => void;
	onSuccess?: () => void;
	holder: TokenHolderUser;
	user: User;
	method: typeof securitySftMulti.transfer | typeof securitySftSingle.transfer;
	tokenIdSize: number;
}
...
const TransferHolderBalancePopup: React.FC<TransferHolderBalancePopupProps> = ({
	open,
	onClose,
	onSuccess,
	holder,
	...
	const { control, handleSubmit, reset } = useForm<TransferFormData>({
		defaultValues: {
			amount: (Number(holder.un_frozen_balance) + Number(holder.frozen_balance)).toString(),
			recipientAddress: user.concordiumAccountAddress,
		},
		mode: "onChange",
	});

	const handleTransferBalance = async (data: TransferFormData) => {
		setError(undefined);
		try {
				...
				[
					{
						...
						amount: data.amount,
						to: { Account: [data.recipientAddress] },
						from: toParamsAddress(holder.holder_address),
						data: "",
					},
				],
				setTxnStatus,
	...
	return (
		<Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
			...
			<DialogContent sx={classes.dialogFormContainer}>
				<form onSubmit={handleSubmit(handleTransferBalance)}>
					<Paper
						elevation={0}
						sx={{
							p: 3,
					...
					>
						<Box display="flex" alignItems="center" mb={2}>
							...
							<Typography variant="subtitle2" color="primary.main" fontWeight={600}>
								You are about to transfer tokens from a holder
							</Typography>
						</Box>

						<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
							This will transfer the specified amount of tokens from the holder to another address.
						</Typography>
					...
					<DialogActions sx={classes.dialogFormActions}>
						...
						<TransactionButton
							...
							txnStatus={txnStatus}
							defaultText="Transfer Balance"
							loadingText="Transferring Balance..."
							size="medium"
						/>
					</DialogActions>
				</form>
...
export default TransferHolderBalancePopup;
