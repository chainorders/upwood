...
import { IndexerService, PagedResponse_InvestmentRecord } from "../../apiClient";
...
export default function InvestmentRecordsTable({ contract_index }: { contract_index: string }) {
	const classes = useCommonStyles();
	const [records, setRecords] = useState<PagedResponse_InvestmentRecord>();
	...
	useEffect(() => {
		if (contract_index) {
			IndexerService.getAdminIndexerInvestmentRecords(
				page,
				pageSize,
				contract_index,
	...
	return (
		<Box>
				...
				<Table>
					<TableBody>
						<TableRow>
							...
							<TableCell sx={classes.tableHeaderCell}>Token Balance</TableCell>
							<TableCell sx={classes.tableHeaderCell}>Currency Balance</TableCell>
							<TableCell sx={classes.tableHeaderCell}>Record Type</TableCell>
							<TableCell sx={classes.tableHeaderCell}>Created</TableCell>
						</TableRow>
						{records?.data?.length ? (
							records.data.map((rec) => (
								<TableRow key={rec.id}>
									<TableCell>{rec.block_height}</TableCell>
									<TableCell>{rec.txn_index}</TableCell>
									<TableCell>{rec.investor}</TableCell>
									<TableCell>{rec.investment_token_id}</TableCell>
									<TableCell>{toDisplayAmount(rec.currency_amount, 6)}</TableCell>
									<TableCell>{rec.token_amount}</TableCell>
									<TableCell>{rec.token_amount_balance}</TableCell>
									<TableCell>{toDisplayAmount(rec.currency_amount_balance, 6)}</TableCell>
									<TableCell>{rec.investment_record_type}</TableCell>
									<TableCell>{formatDateField(rec.create_time)}</TableCell>
								</TableRow>
							))
						) : (
							<TableRow>
			...
			<TablePagination
				component="div"
				count={records?.page_count ? records.page_count * pageSize : 0}
				page={page}
